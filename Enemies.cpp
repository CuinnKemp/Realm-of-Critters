...
void Enemies::checkAlives() {
  ...
  for (int i = 0; i < enemyCounter; i++) {
    ...
    // Runs each individual updateEnemy function for each of the beasts to check if they are alive
...
void Enemies::updateEnemies() {
  ...
  //increments the timer, if it reachers threshold it spawns a new enemy on screen
  timer++;
  if (timer == 100) {
    this->spawnNewEnemy();
    timer = 0;
