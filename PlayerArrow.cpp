...
pArrow::pArrow(Enemies * enemies){
    ...
    if (enemies->enemyCounter >= 3) {
        ...
        for (int t = 0; t < enemies->enemyCounter ; t++){
            distance[t] = sqrt(pow((arrowCoordinates[0] - (enemies->enemies[t]->coordinates[0])),2)
            +pow((arrowCoordinates[0] - (enemies->enemies[t]->coordinates[0])),2));
        }

        // the finds the shortest distance between the player and an enemy
        double shortest = distance[0];
        
        // finds place in enemies array of the enemy with the shoretest distance to the player
        ...
        this->targetCoordinates[0] = enemies->enemies[enemyPlaceInList]->coordinates[0];
        this->targetCoordinates[1] = enemies->enemies[enemyPlaceInList]->coordinates[1];

        //sets the target enemy for other functions use
        closestenemy = enemyPlaceInList;

        //testing to see whether or not it finds the enemy with the shortest distance
        for (int i = 0; i < 50 ;i++) {
...
bool pArrow::EnemycollisionDetection(Enemies * enemies){
    if (enemies->enemies[closestenemy]->sprite.getGlobalBounds().intersects(this->sprite.getGlobalBounds())){
        enemies->enemies[closestenemy]->health = enemies->enemies[closestenemy]->health - this->damage;
        return 1;
    }
    return 0;
}
...
// Movement function, for some reason this doesnt seem to work, its similair to how arrow works
bool pArrow::movement(Enemies * enemies, pArrow * arrow1){
    if (enemies->enemyCounter >= 3) {
        ...
        if (this->EnemycollisionDetection(enemies) == 1 || this->outOfBounds() == 1){
            ...
            arrow1->arrowCoordinates[0] = arrow1->arrowCoordinates[0] + arrow1->speedVector[0];
            arrow1->arrowCoordinates[1] = arrow1->arrowCoordinates[1] + arrow1->speedVector[1];
            arrow1->sprite.setPosition(sf::Vector2f(arrow1->arrowCoordinates[0], arrow1->arrowCoordinates[1]));
    } return 1;
}


// update ability is the same as SpinningBlade, also something wrong here i think
